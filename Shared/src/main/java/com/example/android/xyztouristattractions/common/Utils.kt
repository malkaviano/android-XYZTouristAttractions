...
object Utils {
     ...
     */
    fun distanceBetween(point1: LatLng, point2: LatLng) =
        Math.round(SphericalUtil.computeDistanceBetween(point1, point2)).toDouble()
    ...
    fun getNodes(client: GoogleApiClient): Collection<String> {
        ...
        val nodeResult = Wearable.NodeApi.getConnectedNodes(client).await()

        nodeResult.nodes.forEach {
            results.add(it.id)
        }
