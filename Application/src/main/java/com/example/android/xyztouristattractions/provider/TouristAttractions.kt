...
object TouristAttractions {
    ...
    val CITY_SYDNEY = "Sydney"

    val TEST_CITY = CITY_SYDNEY

    private val TRIGGER_RADIUS = 2000f // 2KM
    private val TRIGGER_TRANSITION = Geofence.GEOFENCE_TRANSITION_ENTER or Geofence.GEOFENCE_TRANSITION_EXIT
    private val EXPIRATION_DURATION = Geofence.NEVER_EXPIRE

    val CITY_LOCATIONS: Map<String, LatLng> = object : HashMap<String, LatLng>() {
        init {
            put(CITY_SYDNEY, LatLng(-33.873651, 151.2068896))
        }
    }
     ...
     */
    val ATTRACTIONS: HashMap<String, List<Attraction>> = object : HashMap<String, List<Attraction>>() {
        init {

            put(CITY_SYDNEY, object : ArrayList<Attraction>() {
                init {
                    add(Attraction(
                            "Sydney Opera House",
    ...
    val geofenceList: List<Geofence>
        get() {
            ...
            for (city in CITY_LOCATIONS.keys) {
                val cityLatLng = CITY_LOCATIONS[city]
    ...
    fun getClosestCity(curLatLng: LatLng?): String {
        if (curLatLng == null) {
        ...
        for ((key, value) in CITY_LOCATIONS) {
            val distance = SphericalUtil.computeDistanceBetween(curLatLng, value)
